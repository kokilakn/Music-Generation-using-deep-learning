...
import os
import os.path
if (os.path.isfile("Piano.csv") == False):
    ...
    import csv
    with open("Piano.csv", "w") as f:
        writer = csv.writer(f)
        writer.writerows(notes_array)
else:
    data_2d = []
    with open('Piano.csv', newline='\n') as f:
...
K.clear_session()
if(os.path.isfile("Piano.h5") == False):
    model=lstm()
    mc=ModelCheckpoint('Piano.h5', monitor='val_loss', mode='min', save_best_only=True,verbose=1)
...
def convert_to_midi(prediction_output):
    ...
    midi_stream.write('midi', fp='Piano.mid')
